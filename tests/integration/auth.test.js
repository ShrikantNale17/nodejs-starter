...
describe('Auth routes', () => {
  ...
  describe('POST /auth/forgot-password', () => {
    ...
    test('should return 204 and send reset password email to the user', async () => {
      ...
      await request(app).post('/auth/forgot-password').send({ email: userOne.email }).expect(httpStatus.NO_CONTENT);
    ...
    test('should return 404 if email does not belong to any user', async () => {
      await request(app).post('/auth/forgot-password').send({ email: userOne.email }).expect(httpStatus.NOT_FOUND);
    });
  });
  ...
  describe('POST /auth/send-verification-email', () => {
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).post('/auth/send-verification-email').send().expect(httpStatus.UNAUTHORIZED);
    });
  });
  ...
  describe('POST /auth/verify-email', () => {
    ...
    test('should return 400 if verify email token is missing', async () => {
      ...
      await request(app).post('/auth/verify-email').send().expect(httpStatus.BAD_REQUEST);
    });
